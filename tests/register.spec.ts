...
//Test registration functionality and registration with existing email
test.describe("Registration Flow", () => {
  ...
  test("User cannot register with the same email", async ({ page }) => {
    ...
    // const er = frame.getByText('The email is already used, please choose another one or sign in', { exact: true })
    // await expect(er).toBeVisible({ timeout: 50000 });
